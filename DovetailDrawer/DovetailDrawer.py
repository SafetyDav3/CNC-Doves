    ...
    def notify(self, args):
        try:
            ...
            for i in range(inputs.count):
                inp = inputs.item(i)
                values[inp.id] = inp.value if hasattr(inp, 'value') else inp.isChecked

            design = adsk.fusion.Design.cast(self.app.activeProduct)
            root = design.rootComponent

            drawer_comp = root.occurrences.addNewComponent(adsk.core.Matrix3D.create()).component
            ...
            bottom_rect.addCenterPointRectangle(adsk.core.Point3D.create(0, 0, 0), p2)
            ...
            def create_side(width, height, thickness, name):
                ...
                lines.addCenterPointRectangle(adsk.core.Point3D.create(0, bottom_offset - height / 2, 0), p2)
            ...
            move_body(left, adsk.core.Vector3D.create(-width/2 + side_thickness/2, 0, bottom_thickness))
            move_body(right, adsk.core.Vector3D.create(width/2 - side_thickness/2, 0, bottom_thickness))
            move_body(front, adsk.core.Vector3D.create(0, -depth/2 + side_thickness/2, bottom_thickness))
            move_body(back, adsk.core.Vector3D.create(0, depth/2 - side_thickness/2, bottom_thickness))
...
def create_command_inputs(cmd):
    ...
    inputs.addValueInput('width', 'Width', 'cm', adsk.core.ValueInput.createByReal(30))
    inputs.addValueInput('height', 'Height', 'cm', adsk.core.ValueInput.createByReal(10))
    inputs.addValueInput('depth', 'Depth', 'cm', adsk.core.ValueInput.createByReal(20))
    inputs.addValueInput('sideThickness', 'Side Thickness', 'cm', adsk.core.ValueInput.createByReal(1))
    inputs.addValueInput('bottomThickness', 'Bottom Thickness', 'cm', adsk.core.ValueInput.createByReal(0.5))
    inputs.addValueInput('tolerance', 'Tolerance', 'cm', adsk.core.ValueInput.createByReal(0.05))
    inputs.addIntegerSpinnerCommandInput('dovetailCount', 'Dovetail Count', 1, 20, 1, 3)
    inputs.addValueInput('dovetailAngle', 'Dovetail Angle', 'deg', adsk.core.ValueInput.createByReal(7))
    inputs.addValueInput('bottomOffset', 'Bottom Offset', 'cm', adsk.core.ValueInput.createByReal(0.5))
    inputs.addStringValueInput('material', 'Material', 'Plywood')
    inputs.addValueInput('toolDiameter', 'Tool Diameter', 'cm', adsk.core.ValueInput.createByReal(0.3))

    inputs.addIntegerSpinnerCommandInput('holeCount', 'Handle Holes', 0, 2, 1, 0)
    inputs.addValueInput('holeSpacing', 'Hole Spacing', 'cm', adsk.core.ValueInput.createByReal(4))
    inputs.addValueInput('holeDiameter', 'Hole Diameter', 'cm', adsk.core.ValueInput.createByReal(0.6))

    inputs.addBoolValueInput('convertComponents', 'Convert to Components', True, '', False)


class DrawerPaletteCommandCreatedEventHandler(adsk.core.CommandCreatedEventHandler):
    def __init__(self, app):
        super().__init__()
        self.app = app
    ...
    def notify(self, args):
        try:
            ...
            self.app.log('Failed to create command inputs:\n{}'.format(traceback.format_exc()))
...
class DrawerAddin:
    ...
    def start(self):
        try:
            ...
            if not self.command_def:
                self.command_def = cmd_definitions.addButtonDefinition('drawerAddin',
                                                                      'Dovetail Drawer',
                                                                      'Create a dovetail drawer box')

            on_command_created = DrawerPaletteCommandCreatedEventHandler(self.app)
            ...
            self.app.log('Failed to start addin:\n{}'.format(traceback.format_exc()))
    ...
    def stop(self):
        try:
            ...
            self.app.log('Failed to stop addin:\n{}'.format(traceback.format_exc()))
